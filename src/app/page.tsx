...
export default function Home() {
  ...
  const handleSummariesResult = (newSummaries: DailySummary[]) => {
  ...
  const handleSummariesRefreshed = (refreshedSummaries: DailySummary[]) => {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                View Git commits and generate daily summaries with Thai date formatting
              </p>
            </div>
        ...
        <GitFilterForm
          onCommitsResult={(newCommits) => {
            ...
            if (newCommits.length > 0) {
              updateFilterParams(newCommits[0].author, new Date(Math.min(...newCommits.map(c => c.date.getTime()))));
            }
          }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="lg:col-span-1">
            <GitCommitsList
              commits={commits}
              loading={loading}
              error={error && error.includes('Git Error') ? error.replace('Git Error: ', '') : ''}
            />
          </div>
          ...
          <div className="lg:col-span-1">
            <DailySummariesView
              summaries={summaries}
              loading={loading}
              error={error && error.includes('Summary Error') ? error.replace('Summary Error: ', '') : ''}
            />
          </div>
        </div>

        {/* Instructions */}
        {commits.length === 0 && summaries.length === 0 && !loading && !error && (
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-700">
                  <ol className="list-decimal list-inside space-y-1">
                    <li>Enter your Git author name (exactly as it appears in commits)</li>
                    ...
                    <li>Use "Refresh Summaries" to update with latest commits</li>
                  </ol>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">
              WFG - Worklog From Git • Built with Next.js, TypeScript, and Tailwind CSS
            </p>
